...
public class Account implements UserDetails {
    ...
    public enum Authority {ROLE_USER, ROLE_MANAGER, ROLE_ADMIN}//Authorityの中に三つの情報が入っている
    ...
    @Temporal(TemporalType.TIMESTAMP)//時間を扱うカラムを定義するときに使用（Temporal）
    private Date createdAt;

    // roleは複数管理できるように、Set<>で定義。
    @ElementCollection(fetch = FetchType.EAGER)
    @Enumerated(EnumType.STRING)//列挙型の中身宣言
    @Column(nullable = false)
